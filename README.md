...
|------|------|
| 左鍵拖曳 | 將寵物移動至螢幕任意位置 |
| 右鍵單擊 | 開啟主選單：活動切換、餵食、商店、背包、番茄鐘、統計、設定 |
...
- **`PetView`** — `overrideredirect=True` 無邊框視窗，使用 `-transparentcolor` 實現透明背景；右鍵選單以 `menu.post()` + `grab_set_global()` 非阻塞方式呈現。
